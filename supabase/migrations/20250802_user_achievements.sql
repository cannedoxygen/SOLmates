...
CREATE OR REPLACE FUNCTION get_user_signup_rank(user_id UUID)
RETURNS INTEGER
LANGUAGE sql
STABLE
AS $$
...
CREATE OR REPLACE FUNCTION check_early_adopter_achievement(user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  SELECT get_user_signup_rank(user_id) INTO signup_rank;
  
  -- Check if achievement already exists
  SELECT EXISTS(
    SELECT 1 FROM user_achievements 
    WHERE user_id = user_id AND achievement_type = 'early_adopter'
  ...
  IF signup_rank <= 100 AND NOT achievement_exists THEN
    ...
    VALUES (user_id, 'early_adopter', jsonb_build_object('signup_rank', signup_rank));
    RETURN TRUE;
  END IF;
...
CREATE OR REPLACE FUNCTION check_collaborator_achievement(user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  SELECT EXISTS(
    SELECT 1 FROM user_achievements 
    WHERE user_id = user_id AND achievement_type = 'collaborator'
  ) INTO achievement_exists;
  
  -- Award achievement if user has 10+ completed projects
  IF project_count >= 10 AND NOT achievement_exists THEN
    INSERT INTO user_achievements (user_id, achievement_type, metadata)
    VALUES (user_id, 'collaborator', jsonb_build_object('project_count', project_count));
    RETURN TRUE;
  END IF;
...
CREATE OR REPLACE FUNCTION check_mentor_achievement(user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  FROM matches m
  JOIN users u ON (
    (m.user1_id = user_id AND u.id = m.user2_id AND u.experience_level = 'Beginner') OR
    (m.user2_id = user_id AND u.id = m.user1_id AND u.experience_level = 'Beginner')
  )
  ...
  SELECT EXISTS(
    SELECT 1 FROM user_achievements 
    WHERE user_id = user_id AND achievement_type = 'mentor'
  ) INTO achievement_exists;
  
  -- Award achievement if user has helped 5+ beginners
  IF mentee_count >= 5 AND NOT achievement_exists THEN
    INSERT INTO user_achievements (user_id, achievement_type, metadata)
    VALUES (user_id, 'mentor', jsonb_build_object('mentee_count', mentee_count));
    RETURN TRUE;
  END IF;
...
CREATE OR REPLACE FUNCTION check_creator_achievement(user_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  FROM projects p
  WHERE p.created_by = user_id AND p.status IN ('completed', 'active')
  INTO innovation_score;
  
  -- Check if achievement already exists
  SELECT EXISTS(
    SELECT 1 FROM user_achievements 
    WHERE user_id = user_id AND achievement_type = 'creator'
  ) INTO achievement_exists;
  
  -- Award achievement if user has used 10+ different technologies
  IF innovation_score >= 10 AND NOT achievement_exists THEN
    INSERT INTO user_achievements (user_id, achievement_type, metadata)
    VALUES (user_id, 'creator', jsonb_build_object('innovation_score', innovation_score));
    RETURN TRUE;
  END IF;
...
BEGIN
  ...
  results := jsonb_set(results, '{early_adopter}', to_jsonb(check_early_adopter_achievement(user_id)));
  results := jsonb_set(results, '{collaborator}', to_jsonb(check_collaborator_achievement(user_id)));
  results := jsonb_set(results, '{mentor}', to_jsonb(check_mentor_achievement(user_id)));
  results := jsonb_set(results, '{creator}', to_jsonb(check_creator_achievement(user_id)));
