.class final Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;
.super Ljava/lang/Object;
.source "IntBufferBatchMountItem.java"

# interfaces
.implements Lcom/facebook/react/fabric/mounting/mountitems/BatchMountItem;


# static fields
.field static final INSTRUCTION_CREATE:I = 0x2

.field static final INSTRUCTION_DELETE:I = 0x4

.field static final INSTRUCTION_FLAG_MULTIPLE:I = 0x1

.field static final INSTRUCTION_INSERT:I = 0x8

.field static final INSTRUCTION_REMOVE:I = 0x10

.field static final INSTRUCTION_UPDATE_EVENT_EMITTER:I = 0x100

.field static final INSTRUCTION_UPDATE_LAYOUT:I = 0x80

.field static final INSTRUCTION_UPDATE_OVERFLOW_INSET:I = 0x400

.field static final INSTRUCTION_UPDATE_PADDING:I = 0x200

.field static final INSTRUCTION_UPDATE_PROPS:I = 0x20

.field static final INSTRUCTION_UPDATE_STATE:I = 0x40

.field static final TAG:Ljava/lang/String;


# instance fields
.field private final mCommitNumber:I

.field private final mIntBuffer:[I

.field private final mIntBufferLen:I

.field private final mObjBuffer:[Ljava/lang/Object;

.field private final mObjBufferLen:I

.field private final mSurfaceId:I


# direct methods
.method static constructor <clinit>()V
    .locals 1

    .line 38
    const-class v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;

    invoke-virtual {v0}, Ljava/lang/Class;->getSimpleName()Ljava/lang/String;

    move-result-object v0

    sput-object v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->TAG:Ljava/lang/String;

    return-void
.end method

.method constructor <init>(I[I[Ljava/lang/Object;I)V
    .locals 1
    .param p1, "surfaceId"    # I
    .param p2, "intBuf"    # [I
    .param p3, "objBuf"    # [Ljava/lang/Object;
    .param p4, "commitNumber"    # I

    .line 62
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 63
    iput p1, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mSurfaceId:I

    .line 64
    iput p4, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mCommitNumber:I

    .line 66
    iput-object p2, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    .line 67
    iput-object p3, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    .line 69
    iget-object v0, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    array-length v0, v0

    iput v0, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBufferLen:I

    .line 70
    iget-object v0, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    array-length v0, v0

    iput v0, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBufferLen:I

    .line 71
    return-void
.end method

.method private static nameForInstructionString(I)Ljava/lang/String;
    .locals 1
    .param p0, "type"    # I

    .line 289
    const/4 v0, 0x2

    if-ne p0, v0, :cond_0

    .line 290
    const-string v0, "CREATE"

    return-object v0

    .line 291
    :cond_0
    const/4 v0, 0x4

    if-ne p0, v0, :cond_1

    .line 292
    const-string v0, "DELETE"

    return-object v0

    .line 293
    :cond_1
    const/16 v0, 0x8

    if-ne p0, v0, :cond_2

    .line 294
    const-string v0, "INSERT"

    return-object v0

    .line 295
    :cond_2
    const/16 v0, 0x10

    if-ne p0, v0, :cond_3

    .line 296
    const-string v0, "REMOVE"

    return-object v0

    .line 297
    :cond_3
    const/16 v0, 0x20

    if-ne p0, v0, :cond_4

    .line 298
    const-string v0, "UPDATE_PROPS"

    return-object v0

    .line 299
    :cond_4
    const/16 v0, 0x40

    if-ne p0, v0, :cond_5

    .line 300
    const-string v0, "UPDATE_STATE"

    return-object v0

    .line 301
    :cond_5
    const/16 v0, 0x80

    if-ne p0, v0, :cond_6

    .line 302
    const-string v0, "UPDATE_LAYOUT"

    return-object v0

    .line 303
    :cond_6
    const/16 v0, 0x200

    if-ne p0, v0, :cond_7

    .line 304
    const-string v0, "UPDATE_PADDING"

    return-object v0

    .line 305
    :cond_7
    const/16 v0, 0x400

    if-ne p0, v0, :cond_8

    .line 306
    const-string v0, "UPDATE_OVERFLOW_INSET"

    return-object v0

    .line 307
    :cond_8
    const/16 v0, 0x100

    if-ne p0, v0, :cond_9

    .line 308
    const-string v0, "UPDATE_EVENT_EMITTER"

    return-object v0

    .line 310
    :cond_9
    const-string v0, "UNKNOWN"

    return-object v0
.end method


# virtual methods
.method public execute(Lcom/facebook/react/fabric/mounting/MountingManager;)V
    .locals 32
    .param p1, "mountingManager"    # Lcom/facebook/react/fabric/mounting/MountingManager;

    .line 75
    move-object/from16 v0, p0

    iget v1, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mSurfaceId:I

    move-object/from16 v2, p1

    invoke-virtual {v2, v1}, Lcom/facebook/react/fabric/mounting/MountingManager;->getSurfaceManager(I)Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;

    move-result-object v1

    .line 76
    .local v1, "surfaceMountingManager":Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;
    if-nez v1, :cond_0

    .line 77
    sget-object v3, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->TAG:Ljava/lang/String;

    iget v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mSurfaceId:I

    .line 80
    invoke-static {v4}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v4

    filled-new-array {v4}, [Ljava/lang/Object;

    move-result-object v4

    .line 77
    const-string v5, "Skipping batch of MountItems; no SurfaceMountingManager found for [%d]."

    invoke-static {v3, v5, v4}, Lcom/facebook/common/logging/FLog;->e(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V

    .line 81
    return-void

    .line 83
    :cond_0
    invoke-virtual {v1}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->isStopped()Z

    move-result v3

    if-eqz v3, :cond_1

    .line 84
    sget-object v3, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->TAG:Ljava/lang/String;

    iget v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mSurfaceId:I

    invoke-static {v4}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v4

    filled-new-array {v4}, [Ljava/lang/Object;

    move-result-object v4

    const-string v5, "Skipping batch of MountItems; was stopped [%d]."

    invoke-static {v3, v5, v4}, Lcom/facebook/common/logging/FLog;->e(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V

    .line 85
    return-void

    .line 87
    :cond_1
    invoke-static {}, Lcom/facebook/react/internal/featureflags/ReactNativeFeatureFlags;->enableFabricLogs()Z

    move-result v3

    if-eqz v3, :cond_2

    .line 88
    sget-object v3, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->TAG:Ljava/lang/String;

    iget v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mSurfaceId:I

    invoke-static {v4}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v4

    const-string v5, "Executing IntBufferBatchMountItem on surface [%d]"

    invoke-static {v3, v5, v4}, Lcom/facebook/common/logging/FLog;->d(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Object;)V

    .line 91
    :cond_2
    const/4 v3, 0x0

    .local v3, "i":I
    const/4 v4, 0x0

    .line 92
    .local v4, "j":I
    :goto_0
    iget v5, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBufferLen:I

    if-ge v3, v5, :cond_11

    .line 93
    iget-object v5, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v6, v3, 0x1

    .end local v3    # "i":I
    .local v6, "i":I
    aget v12, v5, v3

    .line 94
    .local v12, "rawType":I
    and-int/lit8 v13, v12, -0x2

    .line 95
    .local v13, "type":I
    and-int/lit8 v3, v12, 0x1

    const/4 v14, 0x1

    if-eqz v3, :cond_3

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v6, 0x1

    .end local v6    # "i":I
    .local v5, "i":I
    aget v3, v3, v6

    move v6, v5

    goto :goto_1

    .end local v5    # "i":I
    .restart local v6    # "i":I
    :cond_3
    move v3, v14

    :goto_1
    move v15, v3

    .line 97
    .local v15, "numInstructions":I
    const/4 v11, 0x2

    new-array v3, v11, [Ljava/lang/String;

    const-string/jumbo v5, "numInstructions"

    const/16 v16, 0x0

    aput-object v5, v3, v16

    invoke-static {v15}, Ljava/lang/String;->valueOf(I)Ljava/lang/String;

    move-result-object v5

    aput-object v5, v3, v14

    move-object v10, v3

    .line 99
    .local v10, "args":[Ljava/lang/String;
    new-instance v3, Ljava/lang/StringBuilder;

    invoke-direct {v3}, Ljava/lang/StringBuilder;-><init>()V

    const-string v5, "IntBufferBatchMountItem::mountInstructions::"

    invoke-virtual {v3, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    .line 101
    invoke-static {v13}, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->nameForInstructionString(I)Ljava/lang/String;

    move-result-object v5

    invoke-virtual {v3, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    array-length v5, v10

    .line 99
    const-wide/16 v8, 0x0

    invoke-static {v8, v9, v3, v10, v5}, Lcom/facebook/systrace/Systrace;->beginSection(JLjava/lang/String;[Ljava/lang/String;I)V

    .line 104
    const/4 v3, 0x0

    move v7, v3

    move/from16 v17, v4

    move v3, v6

    .end local v4    # "j":I
    .end local v6    # "i":I
    .restart local v3    # "i":I
    .local v7, "k":I
    .local v17, "j":I
    :goto_2
    if-ge v7, v15, :cond_10

    .line 105
    if-ne v13, v11, :cond_5

    .line 106
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v5, v17, 0x1

    .end local v17    # "j":I
    .local v5, "j":I
    aget-object v4, v4, v17

    check-cast v4, Ljava/lang/String;

    invoke-static {v4}, Lcom/facebook/react/fabric/mounting/mountitems/FabricNameComponentMapping;->getFabricComponentName(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v17

    .line 107
    .local v17, "componentName":Ljava/lang/String;
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v6, v3, 0x1

    .end local v3    # "i":I
    .restart local v6    # "i":I
    aget v18, v4, v3

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v4, v5, 0x1

    .end local v5    # "j":I
    .restart local v4    # "j":I
    aget-object v3, v3, v5

    move-object/from16 v19, v3

    check-cast v19, Lcom/facebook/react/bridge/ReadableMap;

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v5, v4, 0x1

    .end local v4    # "j":I
    .restart local v5    # "j":I
    aget-object v3, v3, v4

    move-object/from16 v20, v3

    check-cast v20, Lcom/facebook/react/uimanager/StateWrapper;

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v21, v5, 0x1

    .end local v5    # "j":I
    .local v21, "j":I
    aget-object v3, v3, v5

    move-object/from16 v22, v3

    check-cast v22, Lcom/facebook/react/fabric/events/EventEmitterWrapper;

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v23, v6, 0x1

    .end local v6    # "i":I
    .local v23, "i":I
    aget v3, v3, v6

    if-ne v3, v14, :cond_4

    move/from16 v24, v14

    goto :goto_3

    :cond_4
    move/from16 v24, v16

    :goto_3
    move-object v3, v1

    move-object/from16 v4, v17

    move/from16 v5, v18

    move-object/from16 v6, v19

    move/from16 v18, v7

    .end local v7    # "k":I
    .local v18, "k":I
    move-object/from16 v7, v20

    move-wide/from16 v19, v8

    move-object/from16 v8, v22

    move/from16 v9, v24

    invoke-virtual/range {v3 .. v9}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->createView(Ljava/lang/String;ILcom/facebook/react/bridge/ReadableMap;Lcom/facebook/react/uimanager/StateWrapper;Lcom/facebook/react/fabric/events/EventEmitterWrapper;Z)V

    .line 114
    .end local v17    # "componentName":Ljava/lang/String;
    move-object/from16 v30, v10

    move/from16 v31, v11

    move/from16 v17, v21

    move/from16 v3, v23

    goto/16 :goto_5

    .end local v18    # "k":I
    .end local v21    # "j":I
    .end local v23    # "i":I
    .restart local v3    # "i":I
    .restart local v7    # "k":I
    .local v17, "j":I
    :cond_5
    move/from16 v18, v7

    move-wide/from16 v19, v8

    .end local v7    # "k":I
    .restart local v18    # "k":I
    const/4 v4, 0x4

    if-ne v13, v4, :cond_6

    .line 115
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .local v5, "i":I
    aget v3, v4, v3

    invoke-virtual {v1, v3}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->deleteView(I)V

    move v3, v5

    move-object/from16 v30, v10

    move/from16 v31, v11

    goto/16 :goto_5

    .line 116
    .end local v5    # "i":I
    .restart local v3    # "i":I
    :cond_6
    const/16 v4, 0x8

    if-ne v13, v4, :cond_7

    .line 117
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .restart local v5    # "i":I
    aget v3, v4, v3

    .line 118
    .local v3, "tag":I
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v6, v5, 0x1

    .end local v5    # "i":I
    .restart local v6    # "i":I
    aget v4, v4, v5

    .line 119
    .local v4, "parentTag":I
    iget-object v5, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v7, v6, 0x1

    .end local v6    # "i":I
    .local v7, "i":I
    aget v5, v5, v6

    invoke-virtual {v1, v4, v3, v5}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->addViewAt(III)V

    .line 120
    .end local v3    # "tag":I
    .end local v4    # "parentTag":I
    move v3, v7

    move-object/from16 v30, v10

    move/from16 v31, v11

    goto/16 :goto_5

    .end local v7    # "i":I
    .local v3, "i":I
    :cond_7
    const/16 v4, 0x10

    if-ne v13, v4, :cond_8

    .line 121
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .restart local v5    # "i":I
    aget v3, v4, v3

    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v6, v5, 0x1

    .end local v5    # "i":I
    .restart local v6    # "i":I
    aget v4, v4, v5

    iget-object v5, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v7, v6, 0x1

    .end local v6    # "i":I
    .restart local v7    # "i":I
    aget v5, v5, v6

    invoke-virtual {v1, v3, v4, v5}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->removeViewAt(III)V

    move v3, v7

    move-object/from16 v30, v10

    move/from16 v31, v11

    goto/16 :goto_5

    .line 122
    .end local v7    # "i":I
    .restart local v3    # "i":I
    :cond_8
    const/16 v4, 0x20

    if-ne v13, v4, :cond_9

    .line 123
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .restart local v5    # "i":I
    aget v3, v4, v3

    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v6, v17, 0x1

    .end local v17    # "j":I
    .local v6, "j":I
    aget-object v4, v4, v17

    check-cast v4, Lcom/facebook/react/bridge/ReadableMap;

    invoke-virtual {v1, v3, v4}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->updateProps(ILcom/facebook/react/bridge/ReadableMap;)V

    move v3, v5

    move/from16 v17, v6

    move-object/from16 v30, v10

    move/from16 v31, v11

    goto/16 :goto_5

    .line 124
    .end local v5    # "i":I
    .end local v6    # "j":I
    .restart local v3    # "i":I
    .restart local v17    # "j":I
    :cond_9
    const/16 v4, 0x40

    if-ne v13, v4, :cond_a

    .line 125
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .restart local v5    # "i":I
    aget v3, v4, v3

    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v6, v17, 0x1

    .end local v17    # "j":I
    .restart local v6    # "j":I
    aget-object v4, v4, v17

    check-cast v4, Lcom/facebook/react/uimanager/StateWrapper;

    invoke-virtual {v1, v3, v4}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->updateState(ILcom/facebook/react/uimanager/StateWrapper;)V

    move v3, v5

    move/from16 v17, v6

    move-object/from16 v30, v10

    move/from16 v31, v11

    goto/16 :goto_5

    .line 126
    .end local v5    # "i":I
    .end local v6    # "j":I
    .restart local v3    # "i":I
    .restart local v17    # "j":I
    :cond_a
    const/16 v4, 0x80

    if-ne v13, v4, :cond_c

    .line 127
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .restart local v5    # "i":I
    aget v21, v4, v3

    .line 128
    .local v21, "reactTag":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v4, v5, 0x1

    .end local v5    # "i":I
    .local v4, "i":I
    aget v22, v3, v5

    .line 129
    .local v22, "parentTag":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v4, 0x1

    .end local v4    # "i":I
    .restart local v5    # "i":I
    aget v23, v3, v4

    .line 130
    .local v23, "x":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v4, v5, 0x1

    .end local v5    # "i":I
    .restart local v4    # "i":I
    aget v24, v3, v5

    .line 131
    .local v24, "y":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v4, 0x1

    .end local v4    # "i":I
    .restart local v5    # "i":I
    aget v25, v3, v4

    .line 132
    .local v25, "width":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v4, v5, 0x1

    .end local v5    # "i":I
    .restart local v4    # "i":I
    aget v26, v3, v5

    .line 133
    .local v26, "height":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v27, v4, 0x1

    .end local v4    # "i":I
    .local v27, "i":I
    aget v28, v3, v4

    .line 135
    .local v28, "displayType":I
    invoke-static {}, Lcom/facebook/react/internal/featureflags/ReactNativeFeatureFlags;->setAndroidLayoutDirection()Z

    move-result v3

    if-eqz v3, :cond_b

    .line 136
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v29, v27, 0x1

    .end local v27    # "i":I
    .local v29, "i":I
    aget v27, v3, v27

    .line 137
    .local v27, "layoutDirection":I
    move-object v3, v1

    move/from16 v4, v21

    move/from16 v5, v22

    move/from16 v6, v23

    move/from16 v7, v24

    move/from16 v8, v25

    move/from16 v9, v26

    move-object/from16 v30, v10

    .end local v10    # "args":[Ljava/lang/String;
    .local v30, "args":[Ljava/lang/String;
    move/from16 v10, v28

    move/from16 v31, v11

    move/from16 v11, v27

    invoke-virtual/range {v3 .. v11}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->updateLayout(IIIIIIII)V

    .line 139
    .end local v27    # "layoutDirection":I
    move/from16 v27, v29

    goto :goto_4

    .line 140
    .end local v29    # "i":I
    .end local v30    # "args":[Ljava/lang/String;
    .restart local v10    # "args":[Ljava/lang/String;
    .local v27, "i":I
    :cond_b
    move-object/from16 v30, v10

    move/from16 v31, v11

    .end local v10    # "args":[Ljava/lang/String;
    .restart local v30    # "args":[Ljava/lang/String;
    const/4 v11, 0x0

    move-object v3, v1

    move/from16 v4, v21

    move/from16 v5, v22

    move/from16 v6, v23

    move/from16 v7, v24

    move/from16 v8, v25

    move/from16 v9, v26

    move/from16 v10, v28

    invoke-virtual/range {v3 .. v11}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->updateLayout(IIIIIIII)V

    .line 143
    .end local v21    # "reactTag":I
    .end local v22    # "parentTag":I
    .end local v23    # "x":I
    .end local v24    # "y":I
    .end local v25    # "width":I
    .end local v26    # "height":I
    .end local v28    # "displayType":I
    :goto_4
    move/from16 v3, v27

    goto/16 :goto_5

    .end local v27    # "i":I
    .end local v30    # "args":[Ljava/lang/String;
    .restart local v3    # "i":I
    .restart local v10    # "args":[Ljava/lang/String;
    :cond_c
    move-object/from16 v30, v10

    move/from16 v31, v11

    .end local v10    # "args":[Ljava/lang/String;
    .restart local v30    # "args":[Ljava/lang/String;
    const/16 v4, 0x200

    if-ne v13, v4, :cond_d

    .line 144
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .restart local v5    # "i":I
    aget v4, v4, v3

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v6, v5, 0x1

    .end local v5    # "i":I
    .local v6, "i":I
    aget v5, v3, v5

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v7, v6, 0x1

    .end local v6    # "i":I
    .restart local v7    # "i":I
    aget v6, v3, v6

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v8, v7, 0x1

    .end local v7    # "i":I
    .local v8, "i":I
    aget v7, v3, v7

    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v9, v8, 0x1

    .end local v8    # "i":I
    .local v9, "i":I
    aget v8, v3, v8

    move-object v3, v1

    invoke-virtual/range {v3 .. v8}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->updatePadding(IIIII)V

    move v3, v9

    goto :goto_5

    .line 146
    .end local v9    # "i":I
    .restart local v3    # "i":I
    :cond_d
    const/16 v4, 0x400

    if-ne v13, v4, :cond_e

    .line 147
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .restart local v5    # "i":I
    aget v9, v4, v3

    .line 148
    .local v9, "reactTag":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v4, v5, 0x1

    .end local v5    # "i":I
    .restart local v4    # "i":I
    aget v10, v3, v5

    .line 149
    .local v10, "overflowInsetLeft":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v4, 0x1

    .end local v4    # "i":I
    .restart local v5    # "i":I
    aget v11, v3, v4

    .line 150
    .local v11, "overflowInsetTop":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v4, v5, 0x1

    .end local v5    # "i":I
    .restart local v4    # "i":I
    aget v21, v3, v5

    .line 151
    .local v21, "overflowInsetRight":I
    iget-object v3, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v22, v4, 0x1

    .end local v4    # "i":I
    .local v22, "i":I
    aget v23, v3, v4

    .line 153
    .local v23, "overflowInsetBottom":I
    move-object v3, v1

    move v4, v9

    move v5, v10

    move v6, v11

    move/from16 v7, v21

    move/from16 v8, v23

    invoke-virtual/range {v3 .. v8}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->updateOverflowInset(IIIII)V

    .line 159
    .end local v9    # "reactTag":I
    .end local v10    # "overflowInsetLeft":I
    .end local v11    # "overflowInsetTop":I
    .end local v21    # "overflowInsetRight":I
    .end local v23    # "overflowInsetBottom":I
    move/from16 v3, v22

    goto :goto_5

    .end local v22    # "i":I
    .restart local v3    # "i":I
    :cond_e
    const/16 v4, 0x100

    if-ne v13, v4, :cond_f

    .line 160
    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v3, 0x1

    .end local v3    # "i":I
    .restart local v5    # "i":I
    aget v3, v4, v3

    iget-object v4, v0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v6, v17, 0x1

    .end local v17    # "j":I
    .local v6, "j":I
    aget-object v4, v4, v17

    check-cast v4, Lcom/facebook/react/fabric/events/EventEmitterWrapper;

    invoke-virtual {v1, v3, v4}, Lcom/facebook/react/fabric/mounting/SurfaceMountingManager;->updateEventEmitter(ILcom/facebook/react/fabric/events/EventEmitterWrapper;)V

    move v3, v5

    move/from16 v17, v6

    .line 104
    .end local v5    # "i":I
    .end local v6    # "j":I
    .restart local v3    # "i":I
    .restart local v17    # "j":I
    :goto_5
    add-int/lit8 v7, v18, 0x1

    move-wide/from16 v8, v19

    move-object/from16 v10, v30

    move/from16 v11, v31

    .end local v18    # "k":I
    .local v7, "k":I
    goto/16 :goto_2

    .line 163
    .end local v7    # "k":I
    .restart local v18    # "k":I
    :cond_f
    new-instance v4, Ljava/lang/IllegalArgumentException;

    new-instance v5, Ljava/lang/StringBuilder;

    invoke-direct {v5}, Ljava/lang/StringBuilder;-><init>()V

    const-string v6, "Invalid type argument to IntBufferBatchMountItem: "

    invoke-virtual {v5, v6}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v5

    invoke-virtual {v5, v13}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v5

    const-string v6, " at index: "

    invoke-virtual {v5, v6}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v5

    invoke-virtual {v5, v3}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v5

    invoke-virtual {v5}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v5

    invoke-direct {v4, v5}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v4

    .line 104
    .end local v18    # "k":I
    .end local v30    # "args":[Ljava/lang/String;
    .restart local v7    # "k":I
    .local v10, "args":[Ljava/lang/String;
    :cond_10
    move/from16 v18, v7

    move-wide/from16 v19, v8

    move-object/from16 v30, v10

    .line 167
    .end local v7    # "k":I
    .end local v10    # "args":[Ljava/lang/String;
    .restart local v30    # "args":[Ljava/lang/String;
    invoke-static/range {v19 .. v20}, Lcom/facebook/systrace/Systrace;->endSection(J)V

    .line 168
    .end local v12    # "rawType":I
    .end local v13    # "type":I
    .end local v15    # "numInstructions":I
    .end local v30    # "args":[Ljava/lang/String;
    move/from16 v4, v17

    goto/16 :goto_0

    .line 169
    .end local v17    # "j":I
    .local v4, "j":I
    :cond_11
    return-void
.end method

.method public getSurfaceId()I
    .locals 1

    .line 173
    iget v0, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mSurfaceId:I

    return v0
.end method

.method public isBatchEmpty()Z
    .locals 1

    .line 178
    iget v0, p0, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBufferLen:I

    if-nez v0, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0
.end method

.method public toString()Ljava/lang/String;
    .locals 26

    .line 184
    move-object/from16 v1, p0

    :try_start_0
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    .line 185
    .local v0, "s":Ljava/lang/StringBuilder;
    const-string v2, "IntBufferBatchMountItem [surface:%d]:\n"

    iget v3, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mSurfaceId:I

    invoke-static {v3}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    filled-new-array {v3}, [Ljava/lang/Object;

    move-result-object v3

    invoke-static {v2, v3}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 186
    const/4 v2, 0x0

    .local v2, "i":I
    const/4 v3, 0x0

    .line 187
    .local v3, "j":I
    :goto_0
    iget v4, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBufferLen:I

    if-ge v2, v4, :cond_11

    .line 188
    iget-object v4, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v5, v2, 0x1

    .end local v2    # "i":I
    .local v5, "i":I
    aget v2, v4, v2

    .line 189
    .local v2, "rawType":I
    and-int/lit8 v4, v2, -0x2

    .line 190
    .local v4, "type":I
    and-int/lit8 v6, v2, 0x1

    if-eqz v6, :cond_0

    iget-object v6, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v7, v5, 0x1

    .end local v5    # "i":I
    .local v7, "i":I
    aget v5, v6, v5

    goto :goto_1

    .end local v7    # "i":I
    .restart local v5    # "i":I
    :cond_0
    const/4 v6, 0x1

    move v7, v5

    move v5, v6

    .line 191
    .local v5, "numInstructions":I
    .restart local v7    # "i":I
    :goto_1
    const/4 v6, 0x0

    .local v6, "k":I
    :goto_2
    if-ge v6, v5, :cond_10

    .line 192
    const/4 v8, 0x2

    if-ne v4, v8, :cond_1

    .line 193
    iget-object v8, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v9, v3, 0x1

    .end local v3    # "j":I
    .local v9, "j":I
    aget-object v3, v8, v3

    check-cast v3, Ljava/lang/String;

    invoke-static {v3}, Lcom/facebook/react/fabric/mounting/mountitems/FabricNameComponentMapping;->getFabricComponentName(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v3

    .line 194
    .local v3, "componentName":Ljava/lang/String;
    add-int/lit8 v8, v9, 0x3

    .line 195
    .end local v9    # "j":I
    .local v8, "j":I
    const-string v9, "CREATE [%d] - layoutable:%d - %s\n"

    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v11, v7, 0x1

    .end local v7    # "i":I
    .local v11, "i":I
    aget v7, v10, v7

    .line 198
    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v12, v11, 0x1

    .end local v11    # "i":I
    .local v12, "i":I
    aget v10, v10, v11

    invoke-static {v10}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    filled-new-array {v7, v10, v3}, [Ljava/lang/Object;

    move-result-object v7

    .line 196
    invoke-static {v9, v7}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v7

    .line 195
    invoke-virtual {v0, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 199
    move/from16 v16, v2

    move/from16 v25, v5

    move v3, v8

    move v7, v12

    .end local v3    # "componentName":Ljava/lang/String;
    goto/16 :goto_6

    .end local v8    # "j":I
    .end local v12    # "i":I
    .local v3, "j":I
    .restart local v7    # "i":I
    :cond_1
    const/4 v8, 0x4

    if-ne v4, v8, :cond_2

    .line 200
    const-string v8, "DELETE [%d]\n"

    iget-object v9, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v10, v7, 0x1

    .end local v7    # "i":I
    .local v10, "i":I
    aget v7, v9, v7

    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    filled-new-array {v7}, [Ljava/lang/Object;

    move-result-object v7

    invoke-static {v8, v7}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v7

    invoke-virtual {v0, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move/from16 v16, v2

    move/from16 v25, v5

    move v7, v10

    goto/16 :goto_6

    .line 201
    .end local v10    # "i":I
    .restart local v7    # "i":I
    :cond_2
    const/16 v8, 0x8

    if-ne v4, v8, :cond_3

    .line 202
    const-string v8, "INSERT [%d]->[%d] @%d\n"

    iget-object v9, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v10, v7, 0x1

    .end local v7    # "i":I
    .restart local v10    # "i":I
    aget v7, v9, v7

    .line 204
    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    iget-object v9, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v11, v10, 0x1

    .end local v10    # "i":I
    .restart local v11    # "i":I
    aget v9, v9, v10

    invoke-static {v9}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v12, v11, 0x1

    .end local v11    # "i":I
    .restart local v12    # "i":I
    aget v10, v10, v11

    invoke-static {v10}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    filled-new-array {v7, v9, v10}, [Ljava/lang/Object;

    move-result-object v7

    .line 203
    invoke-static {v8, v7}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v7

    .line 202
    invoke-virtual {v0, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move/from16 v16, v2

    move/from16 v25, v5

    move v7, v12

    goto/16 :goto_6

    .line 205
    .end local v12    # "i":I
    .restart local v7    # "i":I
    :cond_3
    const/16 v8, 0x10

    if-ne v4, v8, :cond_4

    .line 206
    const-string v8, "REMOVE [%d]->[%d] @%d\n"

    iget-object v9, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v10, v7, 0x1

    .end local v7    # "i":I
    .restart local v10    # "i":I
    aget v7, v9, v7

    .line 208
    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    iget-object v9, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v11, v10, 0x1

    .end local v10    # "i":I
    .restart local v11    # "i":I
    aget v9, v9, v10

    invoke-static {v9}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v12, v11, 0x1

    .end local v11    # "i":I
    .restart local v12    # "i":I
    aget v10, v10, v11

    invoke-static {v10}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    filled-new-array {v7, v9, v10}, [Ljava/lang/Object;

    move-result-object v7

    .line 207
    invoke-static {v8, v7}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v7

    .line 206
    invoke-virtual {v0, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;
    :try_end_0
    .catch Ljava/lang/Exception; {:try_start_0 .. :try_end_0} :catch_0

    move/from16 v16, v2

    move/from16 v25, v5

    move v7, v12

    goto/16 :goto_6

    .line 209
    .end local v12    # "i":I
    .restart local v7    # "i":I
    :cond_4
    const/16 v8, 0x20

    const-string v9, "<null>"

    const-string v10, "<hidden>"

    if-ne v4, v8, :cond_7

    .line 210
    :try_start_1
    iget-object v8, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v11, v3, 0x1

    .end local v3    # "j":I
    .local v11, "j":I
    aget-object v3, v8, v3

    .line 212
    .local v3, "props":Ljava/lang/Object;
    sget-boolean v8, Lcom/facebook/react/fabric/FabricUIManager;->IS_DEVELOPMENT_ENVIRONMENT:Z

    if-eqz v8, :cond_5

    .line 213
    if-eqz v3, :cond_6

    invoke-virtual {v3}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v9

    goto :goto_3

    .line 214
    :cond_5
    move-object v9, v10

    :cond_6
    :goto_3
    move-object v8, v9

    .line 215
    .local v8, "propsString":Ljava/lang/String;
    const-string v9, "UPDATE PROPS [%d]: %s\n"

    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v12, v7, 0x1

    .end local v7    # "i":I
    .restart local v12    # "i":I
    aget v7, v10, v7

    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    filled-new-array {v7, v8}, [Ljava/lang/Object;

    move-result-object v7

    invoke-static {v9, v7}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v7

    invoke-virtual {v0, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 216
    move/from16 v16, v2

    move/from16 v25, v5

    move v3, v11

    move v7, v12

    .end local v3    # "props":Ljava/lang/Object;
    .end local v8    # "propsString":Ljava/lang/String;
    goto/16 :goto_6

    .end local v11    # "j":I
    .end local v12    # "i":I
    .local v3, "j":I
    .restart local v7    # "i":I
    :cond_7
    const/16 v8, 0x40

    if-ne v4, v8, :cond_a

    .line 217
    iget-object v8, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    add-int/lit8 v11, v3, 0x1

    .end local v3    # "j":I
    .restart local v11    # "j":I
    aget-object v3, v8, v3

    check-cast v3, Lcom/facebook/react/uimanager/StateWrapper;

    .line 219
    .local v3, "state":Lcom/facebook/react/uimanager/StateWrapper;
    sget-boolean v8, Lcom/facebook/react/fabric/FabricUIManager;->IS_DEVELOPMENT_ENVIRONMENT:Z

    if-eqz v8, :cond_8

    .line 220
    if-eqz v3, :cond_9

    invoke-virtual {v3}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v9

    goto :goto_4

    .line 221
    :cond_8
    move-object v9, v10

    :cond_9
    :goto_4
    move-object v8, v9

    .line 222
    .local v8, "stateString":Ljava/lang/String;
    const-string v9, "UPDATE STATE [%d]: %s\n"

    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v12, v7, 0x1

    .end local v7    # "i":I
    .restart local v12    # "i":I
    aget v7, v10, v7

    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    filled-new-array {v7, v8}, [Ljava/lang/Object;

    move-result-object v7

    invoke-static {v9, v7}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v7

    invoke-virtual {v0, v7}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 223
    move/from16 v16, v2

    move/from16 v25, v5

    move v3, v11

    move v7, v12

    .end local v3    # "state":Lcom/facebook/react/uimanager/StateWrapper;
    .end local v8    # "stateString":Ljava/lang/String;
    goto/16 :goto_6

    .end local v11    # "j":I
    .end local v12    # "i":I
    .local v3, "j":I
    .restart local v7    # "i":I
    :cond_a
    const/16 v8, 0x80

    if-ne v4, v8, :cond_c

    .line 224
    iget-object v8, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v9, v7, 0x1

    .end local v7    # "i":I
    .local v9, "i":I
    aget v7, v8, v7

    .line 225
    .local v7, "reactTag":I
    iget-object v8, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v10, v9, 0x1

    .end local v9    # "i":I
    .restart local v10    # "i":I
    aget v8, v8, v9

    .line 226
    .local v8, "parentTag":I
    iget-object v9, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v11, v10, 0x1

    .end local v10    # "i":I
    .local v11, "i":I
    aget v9, v9, v10

    .line 227
    .local v9, "x":I
    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v12, v11, 0x1

    .end local v11    # "i":I
    .restart local v12    # "i":I
    aget v10, v10, v11

    .line 228
    .local v10, "y":I
    iget-object v11, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v13, v12, 0x1

    .end local v12    # "i":I
    .local v13, "i":I
    aget v11, v11, v12

    .line 229
    .local v11, "w":I
    iget-object v12, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v14, v13, 0x1

    .end local v13    # "i":I
    .local v14, "i":I
    aget v12, v12, v13

    .line 230
    .local v12, "h":I
    iget-object v13, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v15, v14, 0x1

    .end local v14    # "i":I
    .local v15, "i":I
    aget v13, v13, v14

    .line 232
    .local v13, "displayType":I
    invoke-static {}, Lcom/facebook/react/internal/featureflags/ReactNativeFeatureFlags;->setAndroidLayoutDirection()Z

    move-result v14

    if-eqz v14, :cond_b

    iget-object v14, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v16, v15, 0x1

    .end local v15    # "i":I
    .local v16, "i":I
    aget v14, v14, v15

    move/from16 v15, v16

    goto :goto_5

    .end local v16    # "i":I
    .restart local v15    # "i":I
    :cond_b
    const/4 v14, 0x0

    .line 233
    .local v14, "layoutDirection":I
    :goto_5
    move/from16 v16, v2

    .end local v2    # "rawType":I
    .local v16, "rawType":I
    const-string v2, "UPDATE LAYOUT [%d]->[%d]: x:%d y:%d w:%d h:%d displayType:%d layoutDirection: %d\n"

    .line 237
    invoke-static {v8}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v17

    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v18

    invoke-static {v9}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v19

    invoke-static {v10}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v20

    invoke-static {v11}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v21

    invoke-static {v12}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v22

    invoke-static {v13}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v23

    invoke-static {v14}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v24

    move/from16 v25, v5

    .end local v5    # "numInstructions":I
    .local v25, "numInstructions":I
    filled-new-array/range {v17 .. v24}, [Ljava/lang/Object;

    move-result-object v5

    .line 234
    invoke-static {v2, v5}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v2

    .line 233
    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 238
    move v7, v15

    .end local v7    # "reactTag":I
    .end local v8    # "parentTag":I
    .end local v9    # "x":I
    .end local v10    # "y":I
    .end local v11    # "w":I
    .end local v12    # "h":I
    .end local v13    # "displayType":I
    .end local v14    # "layoutDirection":I
    goto/16 :goto_6

    .end local v15    # "i":I
    .end local v16    # "rawType":I
    .end local v25    # "numInstructions":I
    .restart local v2    # "rawType":I
    .restart local v5    # "numInstructions":I
    .local v7, "i":I
    :cond_c
    move/from16 v16, v2

    move/from16 v25, v5

    .end local v2    # "rawType":I
    .end local v5    # "numInstructions":I
    .restart local v16    # "rawType":I
    .restart local v25    # "numInstructions":I
    const/16 v2, 0x200

    if-ne v4, v2, :cond_d

    .line 239
    const-string v2, "UPDATE PADDING [%d]: top:%d right:%d bottom:%d left:%d\n"

    iget-object v5, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v8, v7, 0x1

    .end local v7    # "i":I
    .local v8, "i":I
    aget v5, v5, v7

    .line 242
    invoke-static {v5}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v5

    iget-object v7, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v9, v8, 0x1

    .end local v8    # "i":I
    .local v9, "i":I
    aget v7, v7, v8

    .line 243
    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    iget-object v8, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v10, v9, 0x1

    .end local v9    # "i":I
    .local v10, "i":I
    aget v8, v8, v9

    .line 244
    invoke-static {v8}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    iget-object v9, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v11, v10, 0x1

    .end local v10    # "i":I
    .local v11, "i":I
    aget v9, v9, v10

    .line 245
    invoke-static {v9}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v12, v11, 0x1

    .end local v11    # "i":I
    .local v12, "i":I
    aget v10, v10, v11

    .line 246
    invoke-static {v10}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    filled-new-array {v5, v7, v8, v9, v10}, [Ljava/lang/Object;

    move-result-object v5

    .line 240
    invoke-static {v2, v5}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v2

    .line 239
    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move v7, v12

    goto :goto_6

    .line 247
    .end local v12    # "i":I
    .restart local v7    # "i":I
    :cond_d
    const/16 v2, 0x400

    if-ne v4, v2, :cond_e

    .line 248
    const-string v2, "UPDATE OVERFLOWINSET [%d]: left:%d top:%d right:%d bottom:%d\n"

    iget-object v5, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v8, v7, 0x1

    .end local v7    # "i":I
    .restart local v8    # "i":I
    aget v5, v5, v7

    .line 251
    invoke-static {v5}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v5

    iget-object v7, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v9, v8, 0x1

    .end local v8    # "i":I
    .restart local v9    # "i":I
    aget v7, v7, v8

    .line 252
    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    iget-object v8, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v10, v9, 0x1

    .end local v9    # "i":I
    .restart local v10    # "i":I
    aget v8, v8, v9

    .line 253
    invoke-static {v8}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v8

    iget-object v9, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v11, v10, 0x1

    .end local v10    # "i":I
    .restart local v11    # "i":I
    aget v9, v9, v10

    .line 254
    invoke-static {v9}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v9

    iget-object v10, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v12, v11, 0x1

    .end local v11    # "i":I
    .restart local v12    # "i":I
    aget v10, v10, v11

    .line 255
    invoke-static {v10}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v10

    filled-new-array {v5, v7, v8, v9, v10}, [Ljava/lang/Object;

    move-result-object v5

    .line 249
    invoke-static {v2, v5}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v2

    .line 248
    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move v7, v12

    goto :goto_6

    .line 256
    .end local v12    # "i":I
    .restart local v7    # "i":I
    :cond_e
    const/16 v2, 0x100

    if-ne v4, v2, :cond_f

    .line 257
    add-int/lit8 v3, v3, 0x1

    .line 258
    const-string v2, "UPDATE EVENTEMITTER [%d]\n"

    iget-object v5, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    add-int/lit8 v8, v7, 0x1

    .end local v7    # "i":I
    .restart local v8    # "i":I
    aget v5, v5, v7

    invoke-static {v5}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v5

    filled-new-array {v5}, [Ljava/lang/Object;

    move-result-object v5

    invoke-static {v2, v5}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move v7, v8

    .line 191
    .end local v8    # "i":I
    .restart local v7    # "i":I
    :goto_6
    add-int/lit8 v6, v6, 0x1

    move/from16 v2, v16

    move/from16 v5, v25

    goto/16 :goto_2

    .line 260
    :cond_f
    sget-object v2, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->TAG:Ljava/lang/String;

    new-instance v5, Ljava/lang/StringBuilder;

    invoke-direct {v5}, Ljava/lang/StringBuilder;-><init>()V

    const-string v8, "String so far: "

    invoke-virtual {v5, v8}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v5

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v8

    invoke-virtual {v5, v8}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v5

    invoke-virtual {v5}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v5

    invoke-static {v2, v5}, Lcom/facebook/common/logging/FLog;->e(Ljava/lang/String;Ljava/lang/String;)V

    .line 261
    new-instance v2, Ljava/lang/IllegalArgumentException;

    new-instance v5, Ljava/lang/StringBuilder;

    invoke-direct {v5}, Ljava/lang/StringBuilder;-><init>()V

    const-string v8, "Invalid type argument to IntBufferBatchMountItem: "

    invoke-virtual {v5, v8}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v5

    invoke-virtual {v5, v4}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v5

    const-string v8, " at index: "

    invoke-virtual {v5, v8}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v5

    invoke-virtual {v5, v7}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v5

    invoke-virtual {v5}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v5

    invoke-direct {v2, v5}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v2

    .line 191
    .end local v16    # "rawType":I
    .end local v25    # "numInstructions":I
    .restart local v2    # "rawType":I
    .restart local v5    # "numInstructions":I
    :cond_10
    move/from16 v16, v2

    move/from16 v25, v5

    .line 265
    .end local v2    # "rawType":I
    .end local v4    # "type":I
    .end local v5    # "numInstructions":I
    .end local v6    # "k":I
    move v2, v7

    goto/16 :goto_0

    .line 266
    .end local v7    # "i":I
    .local v2, "i":I
    :cond_11
    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4
    :try_end_1
    .catch Ljava/lang/Exception; {:try_start_1 .. :try_end_1} :catch_0

    return-object v4

    .line 267
    .end local v0    # "s":Ljava/lang/StringBuilder;
    .end local v2    # "i":I
    .end local v3    # "j":I
    :catch_0
    move-exception v0

    .line 271
    .local v0, "e":Ljava/lang/Exception;
    sget-object v2, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->TAG:Ljava/lang/String;

    const-string v3, "Caught exception trying to print"

    invoke-static {v2, v3, v0}, Lcom/facebook/common/logging/FLog;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)V

    .line 273
    new-instance v2, Ljava/lang/StringBuilder;

    invoke-direct {v2}, Ljava/lang/StringBuilder;-><init>()V

    .line 274
    .local v2, "ss":Ljava/lang/StringBuilder;
    const/4 v3, 0x0

    .local v3, "ii":I
    :goto_7
    iget v4, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBufferLen:I

    if-ge v3, v4, :cond_12

    .line 275
    iget-object v4, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mIntBuffer:[I

    aget v4, v4, v3

    invoke-virtual {v2, v4}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    .line 276
    const-string v4, ", "

    invoke-virtual {v2, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    .line 274
    add-int/lit8 v3, v3, 0x1

    goto :goto_7

    .line 278
    .end local v3    # "ii":I
    :cond_12
    sget-object v3, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->TAG:Ljava/lang/String;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Lcom/facebook/common/logging/FLog;->e(Ljava/lang/String;Ljava/lang/String;)V

    .line 280
    const/4 v3, 0x0

    .local v3, "jj":I
    :goto_8
    iget v4, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBufferLen:I

    if-ge v3, v4, :cond_14

    .line 281
    sget-object v4, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->TAG:Ljava/lang/String;

    iget-object v5, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    aget-object v5, v5, v3

    if-eqz v5, :cond_13

    iget-object v5, v1, Lcom/facebook/react/fabric/mounting/mountitems/IntBufferBatchMountItem;->mObjBuffer:[Ljava/lang/Object;

    aget-object v5, v5, v3

    invoke-virtual {v5}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v5

    goto :goto_9

    :cond_13
    const-string/jumbo v5, "null"

    :goto_9
    invoke-static {v4, v5}, Lcom/facebook/common/logging/FLog;->e(Ljava/lang/String;Ljava/lang/String;)V

    .line 280
    add-int/lit8 v3, v3, 0x1

    goto :goto_8

    .line 284
    .end local v3    # "jj":I
    :cond_14
    const-string v3, ""

    return-object v3
.end method
