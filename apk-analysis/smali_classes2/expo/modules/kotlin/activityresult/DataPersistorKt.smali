.class public final Lexpo/modules/kotlin/activityresult/DataPersistorKt;
.super Ljava/lang/Object;
.source "DataPersistor.kt"


# annotations
.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000\u0016\n\u0000\n\u0002\u0010\t\n\u0000\n\u0002\u0010\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0002\u001a\u000c\u0010\u0004\u001a\u00020\u0003*\u00020\u0005H\u0002\u001a\u000e\u0010\u0006\u001a\u0004\u0018\u00010\u0005*\u00020\u0003H\u0002\"\u000e\u0010\u0000\u001a\u00020\u0001X\u0086T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0002\u001a\u00020\u0003X\u0086T\u00a2\u0006\u0002\n\u0000\u00a8\u0006\u0007"
    }
    d2 = {
        "EXPIRATION_TIME",
        "",
        "EXPIRE_KEY",
        "",
        "toBase64",
        "Landroid/os/Bundle;",
        "toBundle",
        "expo-modules-core_debug"
    }
    k = 0x2
    mv = {
        0x1,
        0x9,
        0x0
    }
    xi = 0x30
.end annotation


# static fields
.field public static final EXPIRATION_TIME:J = 0x493e0L

.field public static final EXPIRE_KEY:Ljava/lang/String; = "expire"


# direct methods
.method public static final synthetic access$toBase64(Landroid/os/Bundle;)Ljava/lang/String;
    .locals 1
    .param p0, "$receiver"    # Landroid/os/Bundle;

    .line 1
    invoke-static {p0}, Lexpo/modules/kotlin/activityresult/DataPersistorKt;->toBase64(Landroid/os/Bundle;)Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public static final synthetic access$toBundle(Ljava/lang/String;)Landroid/os/Bundle;
    .locals 1
    .param p0, "$receiver"    # Ljava/lang/String;

    .line 1
    invoke-static {p0}, Lexpo/modules/kotlin/activityresult/DataPersistorKt;->toBundle(Ljava/lang/String;)Landroid/os/Bundle;

    move-result-object v0

    return-object v0
.end method

.method private static final toBase64(Landroid/os/Bundle;)Ljava/lang/String;
    .locals 3
    .param p0, "$this$toBase64"    # Landroid/os/Bundle;

    .line 129
    invoke-static {}, Landroid/os/Parcel;->obtain()Landroid/os/Parcel;

    move-result-object v0

    .local v0, "$this$toBase64_u24lambda_u241":Landroid/os/Parcel;
    const/4 v1, 0x0

    .line 130
    .local v1, "$i$a$-run-DataPersistorKt$toBase64$bytes$1":I
    invoke-virtual {v0, p0}, Landroid/os/Parcel;->writeBundle(Landroid/os/Bundle;)V

    .line 131
    invoke-virtual {v0}, Landroid/os/Parcel;->marshall()[B

    move-result-object v2

    .line 132
    .local v2, "bytes":[B
    invoke-virtual {v0}, Landroid/os/Parcel;->recycle()V

    .line 133
    nop

    .line 129
    .end local v0    # "$this$toBase64_u24lambda_u241":Landroid/os/Parcel;
    .end local v1    # "$i$a$-run-DataPersistorKt$toBase64$bytes$1":I
    .end local v2    # "bytes":[B
    move-object v0, v2

    .line 135
    .local v0, "bytes":[B
    const/4 v1, 0x0

    invoke-static {v0, v1}, Landroid/util/Base64;->encodeToString([BI)Ljava/lang/String;

    move-result-object v1

    const-string v2, "encodeToString(...)"

    invoke-static {v1, v2}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullExpressionValue(Ljava/lang/Object;Ljava/lang/String;)V

    return-object v1
.end method

.method private static final toBundle(Ljava/lang/String;)Landroid/os/Bundle;
    .locals 5
    .param p0, "$this$toBundle"    # Ljava/lang/String;

    .line 117
    const/4 v0, 0x0

    invoke-static {p0, v0}, Landroid/util/Base64;->decode(Ljava/lang/String;I)[B

    move-result-object v1

    .line 118
    .local v1, "bytes":[B
    invoke-static {}, Landroid/os/Parcel;->obtain()Landroid/os/Parcel;

    move-result-object v2

    .local v2, "$this$toBundle_u24lambda_u240":Landroid/os/Parcel;
    const/4 v3, 0x0

    .line 119
    .local v3, "$i$a$-run-DataPersistorKt$toBundle$1":I
    array-length v4, v1

    invoke-virtual {v2, v1, v0, v4}, Landroid/os/Parcel;->unmarshall([BII)V

    .line 120
    invoke-virtual {v2, v0}, Landroid/os/Parcel;->setDataPosition(I)V

    .line 122
    const/4 v0, 0x0

    invoke-virtual {v2, v0}, Landroid/os/Parcel;->readBundle(Ljava/lang/ClassLoader;)Landroid/os/Bundle;

    move-result-object v0

    .line 123
    .local v0, "bundle":Landroid/os/Bundle;
    invoke-virtual {v2}, Landroid/os/Parcel;->recycle()V

    .line 124
    nop

    .line 118
    .end local v0    # "bundle":Landroid/os/Bundle;
    .end local v2    # "$this$toBundle_u24lambda_u240":Landroid/os/Parcel;
    .end local v3    # "$i$a$-run-DataPersistorKt$toBundle$1":I
    return-object v0
.end method
